...
def openai_generate_batch(
    ...
    # GPT-5 and reasoning models (o1, o3, o4) use different parameters
    is_reasoning_model = any(
        x in model_name.lower() for x in ["gpt-5", "o1", "o3", "o4"]
    )

    out = []
    for messages in messages_list:
        if is_reasoning_model:
            # GPT-5 models: only max_completion_tokens, no temperature/top_p
            ...
            # Older models (GPT-4, GPT-4o, etc.): standard parameters
            resp = client.chat.completions.create(
                model=model_name,
                messages=messages,
                temperature=0.7,
